...
```sh
python3 setup-manifests.py $CLUSTER_NAME
```
...
below to create it for the unique name set in the CLUSTER_NAME and the region set in the AZURE_REGION environment
variables.

```sh
az group create --name $CLUSTER_NAME --location $AZURE_REGION
az identity create -g $CLUSTER_NAME -n ${CLUSTER_NAME}_userid
```
...
```sh
az storage account create --location $AZURE_REGION --name sa${CLUSTER_NAME} --kind Storage --resource-group $CLUSTER_NAME --sku Standard_LRS
export ACCOUNT_KEY=`az storage account keys list --account-name sa${CLUSTER_NAME} --resource-group $CLUSTER_NAME --query "[0].value" -o tsv`
```
...
with the VHD stored locally. We must copy and store it in the resource group instead. To do so, first locate the latest RHCOS
image source and export its URL to an environment variable:
...
```sh
az network public-ip create -g $CLUSTER_NAME -n $CLUSTER_NAME --allocation-method static
az network public-ip create -g $CLUSTER_NAME -n ${CLUSTER_NAME}app --allocation-method static

export PUBLIC_IP=`az network public-ip list -g $CLUSTER_NAME --query "[?name=='${CLUSTER_NAME}'] | [0].ipAddress" -o tsv`
export PUBLIC_IP_APPS=`az network public-ip list -g $CLUSTER_NAME --query "[?name=='${CLUSTER_NAME}app'] | [0].ipAddress" -o tsv`
```

### DNS

```sh
az network private-dns zone create -g $CLUSTER_NAME -n ${CLUSTER_NAME}.${BASE_DOMAIN}

az network private-dns record-set srv add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n _etcd-server-ssl._tcp -r 2380 -p 10 -w 10 -t bootstrap-0.${CLUSTER_NAME}.${BASE_DOMAIN}
az network private-dns record-set srv add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n _etcd-server-ssl._tcp -r 2380 -p 10 -w 10 -t control-plane-0.${CLUSTER_NAME}.${BASE_DOMAIN}
az network private-dns record-set srv add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n _etcd-server-ssl._tcp -r 2380 -p 10 -w 10 -t control-plane-1.${CLUSTER_NAME}.${BASE_DOMAIN}
az network private-dns record-set srv add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n _etcd-server-ssl._tcp -r 2380 -p 10 -w 10 -t control-plane-2.${CLUSTER_NAME}.${BASE_DOMAIN}

az network private-dns record-set a add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n bootstrap-0 -a 10.0.0.4
az network private-dns record-set a add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n control-plane-0 -a 10.0.0.5
az network private-dns record-set a add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n control-plane-1 -a 10.0.0.6
az network private-dns record-set a add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n control-plane-2 -a 10.0.0.7
az network private-dns record-set a add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n etcd-0 -a 10.0.0.5
az network private-dns record-set a add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n etcd-1 -a 10.0.0.6
az network private-dns record-set a add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n etcd-2 -a 10.0.0.7

az network private-dns record-set a create -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n api --ttl 60
az network private-dns record-set a create -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n api-int --ttl 60
az network private-dns record-set a add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n api -a 10.0.0.63
az network private-dns record-set a add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n api-int -a 10.0.0.63

az network dns zone create -g $CLUSTER_NAME -n ${CLUSTER_NAME}.${BASE_DOMAIN}

az network dns record-set a create -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n api --ttl 60
az network dns record-set a create -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n *.apps --ttl 300
az network dns record-set a add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n api -a $PUBLIC_IP
az network dns record-set a add-record -g $CLUSTER_NAME -z ${CLUSTER_NAME}.${BASE_DOMAIN} -n *.apps -a $PUBLIC_IP_APPS
```
...
```sh
az vm stop --resource-group $CLUSTER_NAME --name bootstrap-0
az vm deallocate --resource-group $CLUSTER_NAME --name bootstrap-0 --no-wait
