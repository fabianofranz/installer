...
Create DNS records in the *public* zone for the API public load balancer. Make sure the `BASE_DOMAIN_RESOURCE_GROUP` points to the resource group where the public DNS zone exists.
...
Create DNS records in the *private* zone for the internal API load balancer:
...
Create DNS records on the *private* zone for the bootstrap:
...
Create DNS records in the *private* zone for the control plane:
